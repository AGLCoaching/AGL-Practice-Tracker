...
export default async function ClientDashboardPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const coach = client.coach as { first_name: string; last_name: string; company_name: string | null; logo_url: string | null }
          ...
          <div className="space-y-6">
            {metrics.map((metric) => {
              const logs = (metric.logs || []).sort(
                (a: {logged_at: string}, b: {logged_at: string}) => new Date(a.logged_at).getTime() - new Date(b.logged_at).getTime()
              )
              const chartData = logs.map((log: {logged_value: number; logged_at: string}) => ({
                date: format(parseISO(log.logged_at), 'MMM d'),
                value: metric.response_type === 'yesno' ? (log.logged_value ? 1 : 0) : log.logged_value,
              }))
              const avg = logs.length ? (logs.reduce((s: number, l: {logged_value: number}) => s + l.logged_value, 0) / logs.length).toFixed(1) : null
              const best = logs.length ? Math.max(...logs.map((l: {logged_value: number}) => l.logged_value)) : null
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
                          ...
                          {metric.has_goal && metric.goal_start != null && (
                            <ReferenceLine y={metric.goal_start} stroke="#2E75B6" strokeDasharray="4 4"
